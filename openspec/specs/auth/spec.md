...
Autenticação por email/senha com JWT e autorização por papel (RBAC básico): login,
identidade do usuário logado (`get_current_user`) e gate de acesso (`require_role`),
com erros no padrão `{detail, code}`. Base de proteção de todos os endpoints.
